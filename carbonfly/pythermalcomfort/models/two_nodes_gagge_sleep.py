...
def two_nodes_gagge_sleep(
    ...
    **kwargs : dict
    ...
    # Initialize physiological state variables
    state = {
    ...
    t_core_poly = 0.022234 * ((i-1) / 60) ** 2 - 0.27677 * ((i-1) / 60) + 37.02
    ...
    state = _sleep_set(
        ...
        c_dil=c_dil,
        c_str=c_str,
        temp_skin_neutral=state["t_skin"],
        temp_core_neutral=t_core_poly,
        e_skin=state["e_skin"],
        alfa=state["alfa"],
        skin_blood_flow=state["skin_blood_flow"],
        met_shivering=state["met_shivering"],
    )
    return state
